<?
session_start();
$img_arr = array("http://casetaamatic.ru/editor_dev/imgs/design/iphone5_113262.png");
$name	= "Cool man";
...
function create_product($save_arr,$model,$color){
	...
	foreach ($save_arr["tmp_imgs_arr"] as $filepath) {
		...
		file_prepare_directory('public://'.CASE_IMG_DIR, FILE_CREATE_DIRECTORY); //создать папку для хранения изображений, если не существует
		file_prepare_directory('public://'.CASE_IMG_DIR."/".$user->uid, FILE_CREATE_DIRECTORY); //создать папку для изображений пользователя
...
function create_product_view($save_arr){
	...
	foreach($models as $model){
		foreach($colors as $color){
			$node->field_case_product['und'][0]['product_id'] = 1;//create_product($save_arr["tmp_imgs_arr"],$model,$color);
...
//echo "<br/>Debug: ". print_r($save_arr); //раскомментить для отладки
